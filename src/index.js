...
root.render(
    ...
    <BrowserRouter basebame="/app"> 
      <Header />
      <Routes>
        <Route path="/" element={<ThreadList />} /> {/* ルートパスにThreadListコンポーネントを表示 */}
        {/* スレッド一覧画面(スレッド新着の画面) */}

        <Route path="thread/new" element={<ThreadCreateScreen />} /> {/* スレッド作成画面を表示 */}
        {/* 新規スレッド作成画面 */}
...
async function callApi() {
  const res = await fetch( // APIにGETリクエストを送信し、レスポンスを取得
